...
def main(logdir_or_logfile: str, write_pkl: bool, write_csv: bool, out_dir: str):
    ...
    if event_paths:
        ...
        if write_csv:
            ...
            pt = Path(logdir_or_logfile)
            out_file = os.path.join(out_dir, f"{pt.parent}-{pt.name}.csv")
            print(out_file)
...
if __name__ == "__main__":
    ...
    for datasettype_dir in os.listdir(path):
        ...
        for experiment_dir in os.listdir(dataset_path):
            expermient_path = os.path.join(dataset_path, experiment_dir)
            main(expermient_path, False, True, "converted_logs")

    # main(
    #     "/home/piotr/SensorsArticle2022/logs/23.11/real_real",
    #     False,
    #     True,
    #     "converted-log",
    # )
